...
export default function EightQuery( {path} ) {
  ...
  const getByQuantity = () => {
    fetch(path + '/not-prom-products-by-quantity', {
      headers: {
  ...
  const getByName = () => {
    fetch(path + '/not-prom-products-by-name', {
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json',
        Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  return (
    <div className="query-item">
      <div className="d-flex justify-content-between mb-30 align-items-center">
        <div>
          <h3>Запит 8</h3>
          <h5>Скласти список усіх не акційних товарів, відсортованих за кількістю одиниць товару/ за назвою;</h5>
        </div>
